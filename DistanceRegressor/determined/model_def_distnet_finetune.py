...
# custom imports
from Datasets.distnet import DistResNeXt50
...
class DistNetTrial(PyTorchTrial):
    def __init__(self, context: PyTorchTrialContext):
        ...
        self.val_class_criterion = torch.nn.CrossEntropyLoss()
        self.dist_criterion = torch.nn.HuberLoss()
        self.val_dist_criterion = torch.nn.HuberLoss()
        self.key_criterion = torch.nn.MSELoss()
        self.val_key_criterion = torch.nn.MSELoss()
    ...
    def build_model(self):
        ...
        # model.load_state_dict(torch.load('/irad_users/determined/checkpoints/2a2c8a70-9ff1-4359-93a5-dcd9129311e0/state_dict.pth')['models_state_dict'][0], strict=True)
        model.load_state_dict(torch.load('/irad_users/determined/checkpoints/3300d9d6-f061-419c-9b92-84ee6f6d7253/state_dict.pth')['models_state_dict'][0], strict=True)
    ...
    def evaluate_full_dataset(self, data_loader: torch.utils.data.dataloader.DataLoader):
        ...
        with torch.no_grad():
            for i, data in enumerate(data_loader, 0):
                ...
                # loss calculation
                class_loss = self.val_class_criterion(class_preds.squeeze(), classes)
                dist_loss = self.lambda_d*self.val_dist_criterion(distance_preds.squeeze(), distances)
                key_loss = self.lambda_k*self.val_key_criterion(keypoint_preds, torch.cat(boxes)/self.image_size)
                val_loss += class_loss + dist_loss + key_loss
        ...
        return {'Abs Rel': abs_rel_dist,
                ...
                'RMSE': rmse,
                'RMSE log': log_rmse,
                'Val Loss Avg.': val_loss/ len(data_loader)}
